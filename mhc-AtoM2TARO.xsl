...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!-- ======= REPOSITORY CUSTOMIZATION ======= -->
    ...
    <xsl:variable name="taroId" select="ead/archdesc/did/unitid[@label='TARO']"/><!-- Unique 5-digit TAROid identifier for each finding aid is inserted into the converted file using the Alternate Identifier field in AtoM. Modify the @label attribute to match your system settings. If you capture TAROid using another field, review the exported XML file to determine the path to value -->
    <xsl:variable name="repositoryUrl"><xsl:text>http://library.tmc.edu/mcgovern</xsl:text></xsl:variable><!-- Insert your repository URL -->
    <xsl:variable name="taroUser"><xsl:text>hamtmc</xsl:text></xsl:variable> <!-- Insert your taro Abbr name -->
    ...
    <xsl:template match="eadid">
        <eadid countrycode="US" mainagencycode="US-txham">urn:taro:<xsl:value-of select="$taroUser"/>.<xsl:value-of select="$taroId"/> </eadid><!-- Modify @mainagencycode to match your repository -->             
    </xsl:template>
    ...
    <xsl:template name="units-abstract">
        ...
        <xsl:for-each select="unitdate"><!-- Loops through all <unitdate> tags present -->
            ...
            <unitid label="Identification:" countrycode="US" repositorycode="US-txham" encodinganalog="099"><xsl:value-of select="$identifier"/></unitid><!-- Change @repositorycode to match your repository [list of repo codes] -->
    ...
    <xsl:template match="archdesc/appraisal"/><!-- empty. copy and modify based on templates above-->
    ...
    <xsl:template match="archdesc[not(dsc)]">
        <dsc type="combined">
            ...
            <p><extref xmlns:xlink="http://www.w3.org/1999/xlink" xlink:type="simple" xlink:show="new" xlink:actuate="onRequest" xlink:href="https://repo.atom.url/downloads/[coll#-slug].pdf">A preliminary inventory of the collection is available.</extref> </p>
        </dsc>          
    </xsl:template>-->     
